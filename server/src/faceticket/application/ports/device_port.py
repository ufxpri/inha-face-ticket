...
명령 시그니처는 동일하게 유지 (`wake_wristband_wait / signal_pass / signal_deny / clear_wristband`).
...
class IOperatorDevice(Protocol):
    ...
    @property
    def last_wristband_addr(self) -> Optional[str]: ...   # 직전 wake 의 BLE 주소 핸드오프
    ...
    # 실제 기본값은 구현체의 WAKE_WAIT_TIMEOUT_S (operator.py) — 호출 측은 인자 없이 쓴다.
    async def wake_wristband_wait(self, *, timeout_s: float = 15.0) -> bool: ...
